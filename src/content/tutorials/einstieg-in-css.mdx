---
title: 'Dein Einstieg in CSS: Die Grundprinzipien'
description: 'CSS ist die nächste Technologie, die du nach HTML lernen solltest. Mit den Cascading Stylesheets kannst du deine Website gestalten und layouten.'
isDraft: false
...
CSS (Cascading Style Sheets) ist eine Stylesheet-Sprache, mit der du das Erscheinungsbild und die Darstellung einer Website definieren kannst. Sie ermöglicht die Trennung von Inhalten und Design. Konkret bedeutet das, dass HTML die Struktur der Website vorgibt, während CSS die Gestaltung der Inhalte übernimmt. So kannst du das Layout, die Farben, Schriftarten und andere visuelle Aspekte einer Webseite unabhängig von den eigentlichen Inhalten steuern.
...
Eigenschaften bestimmen das Aussehen von HTML-Elementen. In CSS sind diese innerhalb von Regelsätzen (auch CSS-Regeln/Rules genannt) organisiert. Eine Regel besteht aus einem oder mehreren durch Komma (`,`) getrennten Selektoren und einem durch geschweiften Klammern (`{` und `}`) begrenzten Block von Deklarationen.
...
Eine Box hat vier Seiten. Den Himmelsrichtungen folgend und im Uhrzeigersinn: Oben, rechts, unten und links. Diese Seiten lassen sich auch einzeln definieren. Das sieht dann so aus:

<Figure caption="Manche Eigenschaften lassen sich durch sogenannte shorthands auch bündeln.">
  ```css
  /* Definiert alle Seiten einzeln */
  .box {
    padding-top: 0.5em;
    padding-right: 1em;
    padding-bottom: 1em;
  ...
  /* Das gleiche erreichen wir auch so … */
  .box {
  ...
  /* … oder so, wenn alle Seiten gleich groß sind */
  .box {
    padding: 1em;
  }
  ```
