...
description: 'Je umfangreicher deine Website wird, desto unübersichtlicher wird dein CSS. Mache dem ein Ende!'
isDraft: true
...
In die Website binden wir nur eine CSS-Datei ein, nämlich die `main.css`. Diese enthält keine Styles, sondern dient quasi nur als Inhaltsverzeichnis für alle anderen Dateien, die wir hier mittels `@import` einbinden. Schauen wir uns das exemplarisch an:
...
Wie du siehst, werden in der Hauptdatei keine dedizierten Styles mehr definiert. Vielmehr lade ich an dieser Stelle durch die Verwendung von `@use` (Benutze) die Module `base` und `components`. Da es sich hierbei aber eigentlich um Verzeichnisse handelt, wird in diesen automatisch die Datei `_index.scss` gesucht. Im Prinzip handelt es sich dann um ein Inhaltsverzeichnis, in dem die Dateien aufgelistet werden, die wir verwenden möchten.
...
`_font.scss`
: Die Font-Face-Definitionen der lokal eingebundenen Schriftarten, die auch zuerst geladen werden müssen.

`_mixins.scss`
: Das Herzstück aller Sass-Funktionen, die [Mixins](https://sass-lang.com/documentation/at-rules/mixin/), beheimaten mein Breakpoint-Mixin und immer wieder verwendete Styles, die ich an anderer Stelle nur noch mit einem `@import` einbinden muss.

`_variables.scss`
: Enthält alle CSS-Custom-Properties, also die Variablen zu den Fonts, Farben, Größen und Abständen.

`_layout.scss`
: Hier werden alle Basiselemente des Layouts definiert, also der CSS-Reset, die Definition für `html`, `body`, `main` und so weiter.

`_typo.scss`
: Grundsätzliche Basis-Styles für die Typografie, wie zum Beispiel die Definition von `h1` bis `h5`, `p`, `a` und `blockquote`.
