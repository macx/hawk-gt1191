...
cover:
  ...
  caption: 'Typografie am Flughafen. Foto von <a href="https://unsplash.com/@etiennegirardet">Etienne Girardet</a> (Unsplash)'
updates:
  - date: 2026-06-02T09:30:00+01:00
    note: 'Neu: Zeilenfall und Silbentrennung im Abschnitt Textauszeichnung.'
  - date: 2026-06-02T09:30:00+01:00
    note: 'Interaktive Demo erweitert (text-wrap für Headlines, max-inline-size für Fließtext).'
...
Im ersten Teil, [der Mikrotypografie](/tutorials/grundlagen-der-mikrotypografie), haben wir uns mit Zeichen, Abständen und Auszeichnungen im Detail beschäftigt. Auf Layout-Ebene geht es jetzt um den Textfluss im Ganzen: Wie Zeilen fallen, wie Wörter getrennt werden und wie Ligaturen das Satzbild beruhigen.

Typografisch ist das entscheidend, weil ein ruhiger Zeilenfall den Blick führt und die Lesegeschwindigkeit stabil hält. Genau hier setzt die Textauszeichnung im Layout an.

### Witwen und Waisen

Zur Textauszeichnung gehört nicht nur die Wahl der richtigen Zeichen, sondern auch ein sauberer Umbruch im Satzbild. Genau hier kommen **Witwen** und **Waisen** ins Spiel. In älteren Quellen findest du dafür oft noch die historischen Begriffe „Hurenkind“ und „Schusterjunge“.

Gemeint sind zwei verwandte Satzfehler, die den Leserhythmus stören: Eine Witwe ist die letzte Zeile eines Absatzes, die allein am Anfang der nächsten Spalte oder Seite steht. Eine Waise ist die erste Zeile eines Absatzes, die isoliert am Ende der vorherigen Spalte oder Seite steht.

<Figure caption="Beispiel für eine Witwe (historisch: Hurenkind) und eine Waise (historisch: Schusterjunge). Beide solltest du vermeiden.">
  <SnippetHurenkinder title="Kontrast" />
</Figure>

Für einzelne Problemstellen kannst du weiterhin manuell eingreifen, zum Beispiel mit normalen und geschützten Leerzeichen. Wie du diese Zeichen gezielt setzt, findest du im ersten Teil im Abschnitt [Leerzeichen](/tutorials/grundlagen-der-mikrotypografie#leerzeichen).

Für ganze Absätze und Überschriften ist jedoch ein sauberer Zeilenfall die nachhaltigere Lösung. Genau darum geht es im nächsten Abschnitt.

### Zeilenfall und Silbentrennung

Mit Zeilenfall ist in der Typografie gemeint, wie gleichmäßig Zeilen innerhalb eines Absatzes oder einer Überschrift verteilt sind. Ein guter Zeilenfall sorgt für ein ruhiges Schriftbild und reduziert Stolperstellen beim Lesen.

Während wir im [ersten Teil](/tutorials/grundlagen-der-mikrotypografie#umbrüche-und-trennstriche) mit `&shy;` manuelle Sollbruchstellen gesetzt haben, unterstützt du den Browser hier mit automatischen Regeln für ganze Textbereiche. Das ist besonders hilfreich in responsiven Layouts, in denen sich Breiten ständig ändern und feste manuelle Umbrüche schnell wieder brechen:

`text-wrap: wrap;`
: Zeilen werden normal umgebrochen, ohne zusätzliche Optimierung. Das ist der Standard.

`text-wrap: balance;`
: Verteilt den Text auf mehrere Zeilen möglichst ausgewogen. Das ist ideal für Überschriften, damit nicht eine extrem kurze letzte Zeile übrig bleibt.

`text-wrap: pretty;`
: Vermeidet unschöne Umbrüche im Fließtext, zum Beispiel sehr kurze letzte Zeilen bei Absätzen.

`hyphens: auto;`
: Aktiviert automatische Silbentrennung bei langen Wörtern. Das funktioniert sinnvoll, wenn die Sprache korrekt gesetzt ist (z. B. `lang="de"`).

In der Praxis heißt das: Für Überschriften ist `text-wrap: balance` oft die beste Wahl, weil dadurch mehrere Zeilen optisch gleichmäßiger wirken. Für längere Absätze ist `text-wrap: pretty` sinnvoll, weil unschöne Endzeilen reduziert werden. `hyphens: auto` hilft zusätzlich bei schmalen Textspalten oder langen deutschen Wörtern, damit der Block nicht auseinandergezogen wird.

<Figure caption="Praktische Kombination für Headlines und Fließtext im responsiven Layout.">
```css
h1, h2, h3 {
  text-wrap: balance;
}
...
Lass uns die Theorie in die Praxis testen. In der folgenden interaktiven Demo kannst du Schriftgröße, Laufweite und Zeilenhöhe anpassen. Beim `text-wrap` startest du mit `wrap` (Standard) und kannst auf `pretty` oder `balance` wechseln. Dieser Schalter wirkt bewusst auf die Headline. `max-inline-size` steuert separat die Zeilenlänge des Fließtexts.

<Figure caption="Passe Überschrift und Fließtext getrennt an, um die Wirkung von Umbruch und Zeilenlänge besser beurteilen zu können.">
  <SnippetZeilenabstand/>
</Figure>

## Typografie als Gestaltungskraft

Wenn du beide Tutorials zusammen betrachtest, ergibt sich ein klares Bild: Im ersten Teil ging es um Herkunft, Begriffe und mikrotypografische Grundlagen, in diesem Teil um die konkrete Umsetzung im CSS. Genau diese Kombination macht den Unterschied zwischen „Text anzeigen“ und „Text gestalten“.

Vom Buchdruck seit Gutenberg bis zum modernen Web gilt im Kern dieselbe Idee: Guter Satz führt den Blick, schafft Rhythmus und macht Inhalte zugänglich. Ein ruhiger Zeilenfall, sinnvolle Trennung, saubere Abstände und passende Ligaturen geben deinem Layout Struktur, noch bevor Farbe oder Form ins Spiel kommen.

Darum wirken viele starke Designs so aufgeräumt, obwohl sie visuell zurückhaltend sind: Gute Typografie trägt den Entwurf. Wenn das Schriftbild stimmt, brauchst du oft deutlich weniger dekorative Elemente, um eine Website hochwertig wirken zu lassen.
